package modzatsudan.ezstorage.events;
...
import modzatsudan.ezstorage.util.BlockRef;
...
public class SecurityEvents {
    ...
    private void securityCanceler(PlayerInteractEvent e) {
        ...
        if (b instanceof StorageMultiblock) {
            if ((tile = EZStorageUtils.findSecurityBox(
                    new BlockRef(b, e.getPos().getX(), e.getPos().getY(), e.getPos().getZ()), w, null)) != null) {
    ...
    public void onBlockPlaced(PlaceEvent e) {
        List<BlockRef> blocks = EZStorageUtils.getNeighbors(e.getPos().getX(), e.getPos().getY(), e.getPos().getZ(),
                e.getWorld());
        TileEntitySecurityBox tile;
        for (BlockRef b : blocks) {
            if (b.block instanceof StorageMultiblock) {
                if ((tile = EZStorageUtils.findSecurityBox(
                        new BlockRef(b.block, b.pos.getX(), b.pos.getY(), b.pos.getZ()), e.getWorld(),
                        null)) != null) {
