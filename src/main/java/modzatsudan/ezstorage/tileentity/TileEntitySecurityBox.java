...
import modzatsudan.ezstorage.util.SecurityOverrideHelper;
...
public class TileEntitySecurityBox extends TileEntityMultiblock {
    ...
    public boolean isPlayerAllowed(EntityPlayer p) {
        ...
        for (SecurePlayer sp : allowedPlayers) {
            if (sp.id.toString().equals(p.getPersistentID().toString()))
                return true;
        }
    ...
    public void readDataFromNBT(NBTTagCompound tag) {
        ...
        for (int i = 0; i < count; i++) {
            allowedPlayers.add(new SecurePlayer(tag.getUniqueId("allowedPlayer" + i + "_id"),
                    tag.getString("allowedPlayer" + i + "_name")));
        }
    }
