package modzatsudan.ezstorage.old.network;
...
import modzatsudan.ezstorage.old.tileentity.TileEntitySecurityBox.SecurePlayer;
...
public class MessageSecureSync implements IMessage {
    ...
    public void fromBytes(ByteBuf buf) {
        ...
        for (int i = 0; i < count; i++) {
            whitelist.join(new SecurePlayer(tag.getUniqueId("whitelist" + i + "_id"),
                    tag.getString("whitelist" + i + "_name")));
        }
    }
    ...
    public static class Handler implements IMessageHandler<MessageSecureSync, IMessage> {
        ...
        public void handle(MessageSecureSync m) {
            try {
                TileEntitySecurityBox tile = (TileEntitySecurityBox) Minecraft.getMinecraft().world
                        .getTileEntity(m.pos);
